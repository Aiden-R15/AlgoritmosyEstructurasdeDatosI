...
{-
a) Está bien tipado, tomando una dupla de a y b como parametro inicial y aparte cubre todo los casos
b) No está bien tipado ya que la funcion toa una lista de duplas y se intenta definir para un dupla
c) Si esta bien tipado, ya que la funcion toma lista de duplas y (x:xs) es la manera generica de escribir cualquier lista no vacia,
pero no contempla el caso de lista vacia
d) Lo mismo que el caso anterior, pero esta vez solo contempla los casos con listas de 2 elementos o más
e) Está bien tipado, y toma una lista de duplas donde el primer elemento es un entero, pero solo contempla el caso donde el entero sea 0
f) Está bien tipado, pero solo toma el caso en el que el segundo elemento de la dupla sea 1
g) Está bien tipado, ya que toma una función (a) y un entero b, aparte cubre todos los casos
h) Lo mismo que el caso anterior, pero esta vez solo toma el caso en donde el entero es 3
i) No está bien tipado ya que la funcion toma una funcion de int a int y un entero y se le esta dando 3 enteros
-}
